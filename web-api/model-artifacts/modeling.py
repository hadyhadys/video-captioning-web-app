...
class CaptionGenerator(CaptionGeneratorPreTrainedModel):
    def __init__(self, bert_config, visual_config, decoder_config, task_config):
        ...
        if task_config.custom_input_dim is not None:
            self.lp = nn.Linear(task_config.custom_input_dim, visual_config.vocab_size)


        self.apply(self.init_weights)

    def forward(self, video, video_mask=None,
                input_caption_ids=None, decoder_mask=None, action_input=None):

        video_mask = video_mask.view(-1, video_mask.shape[-1])
        video = self.normalize_video(video)

        if action_input is not None:
            if self.task_config.action_feature_global:
                action_input = action_input.expand(-1, video.shape[1], -1)
            video = torch.cat([video, action_input], axis=2)
            video = self.lp(video)
    ...
    def get_visual_output(self, video, video_mask, shaped=False, action_input=None):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
            video = self.normalize_video(video)
            if action_input is not None:
